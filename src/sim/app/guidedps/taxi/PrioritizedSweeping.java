...
public class PrioritizedSweeping extends TaxiAgent {
	...
	public void updatePolicy() {
		State sprime = new State(x, y, model.destination.state,
				model.passenger.state);
		...
		if (model.isTraining()) {
			...
			if (model.world.isEndGame())
				...
				newVal = (1 - alpha) * val + alpha
						* (reward + gamma * maxQ(qTable.get(sprime)));
			}
	...
	private Action ActionSelection() {
		...
		if (!model.isTraining()) {
			return Action.values()[Utils
					.bestAction(model.random, qTable.get(s))];
		} else {
			// return Action.values()[Utils.boltzmannSelection(model.random,
			// temperature, qTable.get(s))];
			return Action.values()[Utils.epsilonGreedy(model.random, epsilon,
					qTable.get(s))];
		}
			...
			for (int y = 0; y < 5; ++y) {
											...
											// to 3
				{
												...
												// to 4
					{
						...
						qTable.put(new State(x, y, LocState.values()[d],
								LocState.values()[p]), temp);
					}
				}
			}
		}
